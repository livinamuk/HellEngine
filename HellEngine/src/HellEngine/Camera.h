...
public:
	...
	{
		glm::vec3 p = Position;
		p.y = viewHeight + CalculateHeadBob();
		return p;
	}
	...
	{
		glm::vec3 p = GetViewPosition();
		viewMatrix = glm::lookAt(p, p + Front, Up);
	...
	void CalculateProjectionMatrix(int screenWidth, int screenHeight)
	{
		...
		projectionMatrix = glm::perspective(1.00f, (float)screenWidth / (float)screenHeight, 0.05f, 100.0f);
	}
	...
	void Update(float deltaTime)
	{
		...
		// Movement
		if (HellEngine::Input::IsKeyPressed(HELL_KEY_W))
			this->ProcessKeyboard(Camera_Movement::FORWARD, deltaTime);
		if (HellEngine::Input::IsKeyPressed(HELL_KEY_S))
			this->ProcessKeyboard(Camera_Movement::BACKWARD, deltaTime);
		if (HellEngine::Input::IsKeyPressed(HELL_KEY_A))
			this->ProcessKeyboard(Camera_Movement::LEFT, deltaTime);
		if (HellEngine::Input::IsKeyPressed(HELL_KEY_D))
