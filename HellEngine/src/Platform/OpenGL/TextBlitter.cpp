...
{
	unsigned int HellEngine::TextBlitter::VAO = 0;

	void HellEngine::TextBlitter::RenderQuad(Shader* shader)
	{
		if (VAO == 0)
		{
			float quadVertices[] = {
	...
	void HellEngine::TextBlitter::RenderCrosshair(Shader* shader, int screenWidth, int screenHeight, int crosshairSize)
	{
...
#include "HellEngine/TextBlitter.h"

namespace HellEngine
{
	//unsigned int HellEngine::TextBlitter::VAO = -1;

	void TextBlitter::Blit(Shader* shader, std::string text)
	{
		//if (VAO == -1)
		//	glGenVertexArrays(1, &VAO);
