...
namespace HellEngine
{
	...
	public:
		...
		void SetupHardcodedObjects()
		{
			...
			glm::vec3 noteScale = glm::vec3(0.2f, 0.1f, 0.1f);
			glm::vec3 noteRotation = glm::vec3(0, 2.5f, 0);
			glm::vec3 notePosition = glm::vec3(3.55f, 0.01f, 8.9f);

			RenderableObject::NewObject("Note", Model::GetByName("UnitPlane.obj"), &physics);

			RenderableObject::SetPositionByName("Note", notePosition);
			RenderableObject::SetRotationByName("Note", noteRotation);

			RenderableObject::SetDiffuseTextureByName("Note", Texture::GetIDByName("Note_BaseColor.png"));
			RenderableObject::SetRoughnessTextureByName("Note", Texture::GetIDByName("Black.png"));
			RenderableObject::SetMetallicTextureByName("Note", Texture::GetIDByName("Black.png"));
			RenderableObject::SetNormalMapByName("Note", Texture::GetIDByName("Hands_NormalMap.png"));
			RenderableObject::SetScaleByName("Note", noteScale);
		
			glm::vec3 noteBB = glm::vec3(3.5f, 0.01f, 8.8f);
			noteScale.y = 0.05f;
			BoundingBox bb9977 = BoundingBox(noteBB, noteScale, ObjectOrigin::CENTERED, "Note", false);
			//bb9977.SetAngle(2.5f);
			RenderableObject::SetBoundingBoxByName("PictureFrame", bb9977);
			...
			StaticEntity enitty(Model::GetByName("Old_Cotton_Couch.obj"), glm::vec3(0, 0, 0), "COUCH");
			enitty.SetPosition(glm::vec3(-2.9f, 0, 6.1f));
			enitty.SetRotation(glm::vec3(0, HELL_PI, 0));
			enitty.SetScale(glm::vec3(0.07));
			enitty.material = Material::GetMaterialByName("Couch");
			staticEntities.push_back(enitty);

			enitty = StaticEntity(Model::GetByName("PictureFrame.FBX"), glm::vec3(0, 0, 0), "PICTURE_FRAME_2");
			enitty.SetPosition(glm::vec3(2.2f, 1.6f, 6.7));
			enitty.SetRotation(glm::vec3(ROTATE_270, 0, ROTATE_90));
			enitty.SetScale(glm::vec3(0.87f));
			enitty.material = Material::GetMaterialByName("PictureFrame2");
			staticEntities.push_back(enitty);

			enitty = StaticEntity(Model::GetByName("PictureFrame.FBX"), glm::vec3(0, 0, 0), "PICTURE_FRAME");
			enitty.SetPosition(glm::vec3(0, 1.6f, 3.25f));
			enitty.SetRotation(glm::vec3(ROTATE_270, ROTATE_90, 0));
			enitty.SetScale(glm::vec3(0.9));
			enitty.material = Material::GetMaterialByName("PictureFrame");
			staticEntities.push_back(enitty);

			enitty = StaticEntity(Model::GetByName("Key.fbx"), glm::vec3(0, 0, 0), "KEY");
			enitty.SetPosition(glm::vec3(-3.75f, 0.05f, 0.8f));
			enitty.SetRotation(glm::vec3(ROTATE_270, 5.5f, 0));
			enitty.SetScale(glm::vec3(0.0015));
			enitty.material = Material::GetMaterialByName("Key");
			staticEntities.push_back(enitty);
		}
		...
		void MainRenderLoop()
		{
			...
			glm::vec3 out_end = camera.transform.position + (camera.Front * glm::vec3(-100));
			...
			bulletRaycastData.index = -1;
			bulletRaycastData.distance = 0;
			bulletRaycastData.name = "UNKNOWN";
			...
			if (RayCallback.hasHit()) 
			{
				...
				for (int i = 0; i < physics.m_rigidBodies.size(); i++)
				{
					...
					{
						btRigidBody* rigidBody = physics.m_rigidBodies[i];
						bulletRaycastData.distance = (RayCallback.m_hitPointWorld - Util::glmVec3_to_btVec3(camera.transform.position)).length();

						EntityData* entityData = (EntityData*)rigidBody->getUserPointer();
						if (entityData) {
							bulletRaycastData.index = entityData->vectorIndex;
							bulletRaycastData.name = entityData->name;
...
;
		...
		void CrosshairPass(Shader *shader, int crosshairSize)
		{
			...
			if (bulletRaycastData.distance < interactDistance)
			{
				if (bulletRaycastData.name == "DOOR"
					|| (!Door::PlayerHasKey && bulletRaycastData.name == "KEY")
					|| (bulletRaycastData.name == "NOTE"))
		...
		void DrawScene(Shader* shader, bool bindTextures)
		{
			...
			if (bindTextures)
			{
				...
				Util::OUTPUT("RAYCAST: " + bulletRaycastData.name);
				Util::OUTPUT("Index:   " + std::to_string(bulletRaycastData.index));
		...
		void Interact()
		{
			...
			{
				if (bulletRaycastData.name == "DOOR")
					house.doors[bulletRaycastData.index].Interact(player.position);
			}
			
		

			/*if (raycastData.distance < interactDistance)
			{
...
//				b.push_back(&d.boundingBox);
		...
		RaycastData GetRaycastData(float variance)
		{
			...
			glm::vec3 ray_origin = camera.transform.position;
